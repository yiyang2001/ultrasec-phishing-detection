...
lr_accuracy_list, lr_precision_list, lr_recall_list = [], [], []
...
for i in range(0, K):
    # ----- Linear Regression ----- #
    lr_model.fit(X_train_list[i], Y_train_list[i])
    lr_predictions = lr_model.predict(X_test_list[i])
    tn, fp, fn, tp = confusion_matrix(y_true=Y_test_list[i], y_pred=lr_predictions).ravel()
    lr_accuracy, lr_precision, lr_recall = calculate_measures(tn, tp, fn, fp)
...
LR_accuracy = sum(lr_accuracy_list) / len(lr_accuracy_list)
LR_precision = sum(lr_precision_list) / len(lr_precision_list)
LR_recall = sum(lr_recall_list) / len(lr_recall_list)
...
data = {'accuracy': [LR_accuracy,NB_accuracy, SVM_accuracy, DT_accuracy, RF_accuracy, AB_accuracy, NN_accuracy, KN_accuracy],
        'precision': [LR_accuracy,NB_precision, SVM_precision, DT_precision, RF_precision, AB_precision, NN_precision, KN_precision],
        'recall': [LR_accuracy,NB_recall, SVM_recall, DT_recall, RF_recall, AB_recall, NN_recall, KN_recall]
        }

index = ['LR','NB', 'SVM', 'DT', 'RF', 'AB', 'NN', 'KN']
